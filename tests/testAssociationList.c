...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (!AList_insert(&dict, (void *)new_data, (void *)new_key, compareInts, NULL)) {
            printf("\nFailed inserting node (key=%d, data=%d)\n", *new_key, *new_data);
            free(new_data);
            free(new_key);
        } else {
            printf("\n==== Printing list after inserting node (key=%d, data=%d) ====\n", *new_key, *new_data);
            AList_traverse(dict, printListItem, NULL);
            putchar('\n');
        }
    }
    ...
    printf("\n\n!! STARTING DELETIONS !!\n");
    for (int i = 1; i < 41; i++) {
        KeyValuePair deleted = AList_deleteByKey(&dict, (void *)&i, compareInts, NULL);
    ...
    for (int i = 0; i < 6; i++) {
        ...
        if (!AList_append(&dict, (void *)new_data, (void *)new_key, compareInts, NULL)) {
    ...
    AList_destroy(&dict, freeKeyValuePair, NULL);
    return 0;
}
